$(document).ready(function () {
  ...
  let productForm = {
    onProductOptionChanged: function (event) {
      ...
      // Pass selected variant variable when form change
    ...
    getActiveVariant: function ($form) {
      ...
      // Return selectedVariant only all 3 options selected
    ...
    validate: function (event, selectedVariant) {
      // Check if selectedVariant and send variant product id to cart, disable addtocart button
      let $form = $(this),
        ...
        // Change price based on selectedVariant
      ...
      // Add money span to html and convert the currency
  ...
  // Line item: Remove item when quantity input is 0, remove item on remove click
  let removeLineSelector = ".js-remove-line",
    lineQuantitySelector = ".js-line-quantity",
    lineItem = {
      isInMiniCart: function (element) {
        let $element = $(element),
          $miniCart = $element.closest(".js-mini-cart-contents"),
